...
class BookspiderSpider(scrapy.Spider):
    ...
    def parse_book(self, response):
        yield {
            ...
            "price": float(response.css("p.price_color::text").get().strip("£")),
            "amount_in_stock": int(response.css("p.instock.availability").get().split()[-3].strip("(")),
            "rating": self.rating.get(response.css("p.star-rating::attr(class)").get().split()[-1]),
